...
- **Simulace IET a RET:** Program generuje slovo, jež odpovídá kódování příslušné fázové trajektorie pro výměny dvou nebo tří intervalů. V případě výměny obdélníků platí totez, ale jen pro výměnu tří obdélníků.
- **Geometrická skew transformace:** Aplikace transformace $\phi(x, y) = (x, y - (1 - \beta)x)$ pro zobrazení systému na torus a analýzu struktury úseček po jejich transformaci.
...
## Struktura projektu
- `main.py`: Vstupní bod pro spuštění a testování jednotlivých funkcí programu Interval_Exchange_Transformation.
- `Interval_Exchange_Transformation.py`: Program odpovídající za samotnou výměnu intervalů (IET), generování slov a vykreslování.
- `Rectangle_Exchange_Transformation.py`: Program pro vykreslení fázové trajektorie výměny tří obdélníků a generování odpovídajících kódování daných trajektorií.
- `RET_Animation.py`: Interaktivní animace RET transformací a rotací.
- `RET_Map_phi.py`: Program pro podrobnou geometrickou analýzu skew zobrazení $\phi$. Provádí symbolický výpočet a srovnání přesných délek úseček orbity v původním fázovém prostoru a v transformované množině na toru. Výstupem je dvojice porovnávacích grafů (orbita vs. množina $P$) včetně transformovaných dělících hranic a mřížky.

## Požadavky
Projekt vyžaduje Python 3.x a následující knihovny:
numpy, matplotlib, sympy, tqdm
